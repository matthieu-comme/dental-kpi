...
def test_export_sans_token_retourne_401():
    assert (
        client.get(URL, params={"resource": "devis", "columns": "id_devis"}).status_code
        == 401
    )
...
def test_export_ressource_inconnue(sec_headers):
    r = client.get(
        URL, params={"resource": "inconnu", "columns": "id"}, headers=sec_headers
    )
...
def test_export_colonne_inconnue(sec_headers):
    r = client.get(
        URL,
        params={"resource": "devis", "columns": "champ_inexistant"},
        headers=sec_headers,
    )
    assert r.status_code == 400
...
def test_export_colonnes_vides(sec_headers):
    r = client.get(
        URL, params={"resource": "devis", "columns": ""}, headers=sec_headers
    )
    assert r.status_code == 400
...
def test_export_devis_vide(sec_headers):
    r = client.get(
        URL,
        params={"resource": "devis", "columns": "id_devis,montant"},
        headers=sec_headers,
    )
...
def test_export_devis_colonnes_selectionnees(sec_headers, prat_headers, praticien_id):
    _create_devis(praticien_id, prat_headers)
    r = client.get(
        URL, params={"resource": "devis", "columns": "id_devis"}, headers=sec_headers
    )
    headers, rows = _parse_csv(r.content)
...
def test_export_cheques_colonne_praticien_affiche_nom(sec_headers, prat_headers, praticien_id):
    """La colonne 'Praticien' affiche le nom, pas l'ID numérique."""
    _create_cheque(praticien_id, sec_headers)
    r = client.get(
        URL,
        params={"resource": "cheques", "columns": "id_praticien,montant"},
        headers=sec_headers,
    )
    assert r.status_code == 200
    _, rows = _parse_csv(r.content)
    assert len(rows) == 1
    assert rows[0]["Praticien"] == "Dr. Test"


def test_export_journees_colonne_praticien_affiche_nom(sec_headers, praticien_id):
    """La colonne 'Praticien' affiche le nom, pas l'ID numérique."""
    _create_journee(praticien_id, sec_headers)
    r = client.get(
        URL,
        params={"resource": "journees", "columns": "id_praticien,date_jour"},
        headers=sec_headers,
    )
    assert r.status_code == 200
    _, rows = _parse_csv(r.content)
...
def test_export_charges_colonne_praticien_affiche_nom(sec_headers, prat_headers, praticien_id):
    """La colonne 'Praticien' affiche le nom, pas l'ID numérique."""
    _create_charge(praticien_id, prat_headers)
    r = client.get(
        URL,
        params={"resource": "charges", "columns": "id_praticien,designation"},
        headers=sec_headers,
    )
    assert r.status_code == 200
    _, rows = _parse_csv(r.content)
...
def test_export_valeur_none_serialisee_en_chaine_vide(
...
def test_export_enum_serialise_en_valeur(sec_headers, prat_headers, praticien_id):
    ...
    _create_devis(praticien_id, prat_headers)
    r = client.get(
        URL, params={"resource": "devis", "columns": "statut"}, headers=sec_headers
    )
    _, rows = _parse_csv(r.content)
    assert rows[0]["Statut"] == "EN_ATTENTE"
...
def test_export_praticien_voit_seulement_ses_donnees(
    prat_headers, praticien_id, sec_headers
):
    ...
    r = client.get(
        URL,
        params={"resource": "devis", "columns": "id_praticien"},
...
def test_export_secretaire_voit_tous_les_devis(prat_headers, praticien_id, sec_headers):
    client.post(
        "/api/v1/praticiens/",
        json={"nom": "Dr. Autre2", "pin_clair": "333333"},
        headers=sec_headers,
    )
    db = TestingSessionLocal()
    autre = (
        db.query(models.Praticien).filter(models.Praticien.nom == "Dr. Autre2").first()
    )
    assert autre is not None
    autre_id = autre.id_praticien
    db.close()

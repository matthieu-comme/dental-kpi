...
def test_update_config(sec_headers):
    r = client.put(
        "/api/v1/systeme/config",
        json={"nom_cabinet": "Nouveau Cabinet"},
        headers=sec_headers,
    )
    assert r.status_code == 200
...
def test_update_config_praticien_forbidden(prat_headers):
    r = client.put(
        "/api/v1/systeme/config", json={"nom_cabinet": "Hack"}, headers=prat_headers
    )
...
def test_create_log(sec_headers):
    r = client.post(
        "/api/v1/systeme/logs",
        json={"type_action": "AJOUT_DEVIS", "details": "test"},
        headers=sec_headers,
    )
...
def test_create_log_no_auth():
    r = client.post(
        "/api/v1/systeme/logs", json={"type_action": "AJOUT_DEVIS", "details": "test"}
    )
    assert r.status_code == 401
...
def test_create_praticien_sans_token_retourne_401():
    r = client.post(
        "/api/v1/praticiens/", json={"nom": "Dr. Hack", "pin_clair": "111111"}
    )
    assert r.status_code == 401
...
def test_update_praticien(praticien_id, sec_headers):
    r = client.put(
        f"/api/v1/praticiens/{praticien_id}",
        json={"nom": "Dr. Modifié"},
        headers=sec_headers,
    )
    assert r.status_code == 200
...
def test_praticien_cannot_access_other_praticien(praticien_id, sec_headers):
    ...
    r = client.get(
        f"/api/v1/praticiens/{autre_id}", headers={"Authorization": f"Bearer {token}"}
    )
    assert r.status_code == 403
...
def test_read_parametres_praticien(prat_headers, praticien_id):
    r = client.get(
        f"/api/v1/praticiens/{praticien_id}/parametres", headers=prat_headers
    )
    assert r.status_code == 200
...
def test_update_parametres_praticien(prat_headers, praticien_id):
    r = client.put(
        f"/api/v1/praticiens/{praticien_id}/parametres",
        json={"taux_horaire_cible": 400.0},
        headers=prat_headers,
    )
    assert r.status_code == 200
...
def test_create_devis_praticien(prat_headers, praticien_id):
    r = client.post(
        "/api/v1/devis/", json=_devis_payload(praticien_id), headers=prat_headers
    )
    assert r.status_code == 201
...
def test_update_devis_not_found(sec_headers):
    r = client.put(
        "/api/v1/devis/9999", json={"statut": "ACCEPTE"}, headers=sec_headers
    )
    assert r.status_code == 404
...
def test_praticien_cannot_access_other_devis(praticien_id, sec_headers):
    ...
    devis = client.post(
        "/api/v1/devis/", json=_devis_payload(autre_id), headers=sec_headers
    ).json()

    token = create_access_token({"sub": str(praticien_id), "role": "praticien"})
    r = client.get(
        f"/api/v1/devis/{devis['id_devis']}",
...
def test_create_cheque(sec_headers, praticien_id):
    r = client.post(
        "/api/v1/cheques/", json=_cheque_payload(praticien_id), headers=sec_headers
    )
    assert r.status_code == 201
...
def test_update_cheque(sec_headers, praticien_id):
    ...
    r = client.put(
        f"/api/v1/cheques/{created['id_cheque']}",
        json={"statut": "DEPOSE"},
        headers=sec_headers,
    )
    assert r.status_code == 200
...
def test_praticien_cannot_access_other_cheque(praticien_id, sec_headers):
    ...
    cheque = client.post(
        "/api/v1/cheques/", json=_cheque_payload(autre_id), headers=sec_headers
    ).json()
    token = create_access_token({"sub": str(praticien_id), "role": "praticien"})
    r = client.get(
        f"/api/v1/cheques/{cheque['id_cheque']}",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert r.status_code == 403
...
def test_create_journee_sans_token_retourne_401(praticien_id):
...
def test_update_journee(sec_headers, praticien_id):
    ...
    r = client.put(
        f"/api/v1/journees/{created['id_journee']}",
        json={"nb_patients_vus": 15},
        headers=sec_headers,
    )
    assert r.status_code == 200
...
def test_create_charge_praticien(prat_headers, praticien_id):
    r = client.post(
        "/api/v1/charges/", json=_charge_payload(praticien_id), headers=prat_headers
    )
    assert r.status_code == 201
...
def test_create_charge_secretaire_forbidden(sec_headers, praticien_id):
    r = client.post(
        "/api/v1/charges/", json=_charge_payload(praticien_id), headers=sec_headers
    )
    assert r.status_code == 403
...
def test_update_charge(prat_headers, praticien_id):
    ...
    r = client.put(
        f"/api/v1/charges/{created['id_charge']}",
        json={"montant": 1200.0},
        headers=prat_headers,
    )
    assert r.status_code == 200
...
def test_update_charge_not_found(prat_headers):
    r = client.put(
        "/api/v1/charges/9999", json={"montant": 100.0}, headers=prat_headers
    )
    assert r.status_code == 404
...
def test_update_performance(prat_headers):
    ...
    r = client.put(
        f"/api/v1/performances/{created['id_perf']}",
        json={"ca_declare": 11000.0},
        headers=prat_headers,
    )
    assert r.status_code == 200
...
def test_update_performance_not_found(prat_headers):
    r = client.put(
        "/api/v1/performances/9999", json={"ca_declare": 1000.0}, headers=prat_headers
    )
    assert r.status_code == 404
...
def test_read_deviss_filter_statut(sec_headers, praticien_id):
    ...
    payload_acc = {
        **_devis_payload(praticien_id),
        "statut": "ACCEPTE",
        "date_decision": "2023-01-02",
    }
...
def test_read_deviss_filter_patient_ilike(sec_headers, praticien_id):
    ...
    r = client.get(
        "/api/v1/devis/", params={"id_patient": "p_test"}, headers=sec_headers
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_read_deviss_filter_date_range(sec_headers, praticien_id):
    ...
    r = client.get(
        "/api/v1/devis/",
        params={"date_from": "2023-06-01", "date_to": "2023-12-31"},
        headers=sec_headers,
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_read_deviss_filter_montant(sec_headers, praticien_id):
    ...
    r = client.get(
        "/api/v1/devis/",
        params={"montant_min": "200", "montant_max": "1000"},
        headers=sec_headers,
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_read_deviss_filter_no_match(sec_headers, praticien_id):
    ...
    r = client.get(
        "/api/v1/devis/", params={"id_patient": "INEXISTANT"}, headers=sec_headers
    )
    assert r.status_code == 200
    assert r.json() == []
...
def test_read_deviss_praticien_scope(prat_headers, sec_headers, praticien_id):
    ...
    db = TestingSessionLocal()
    autre = (
        db.query(models.Praticien).filter(models.Praticien.nom == "Dr. ScopeD").first()
    )
    assert autre is not None
    autre_id = autre.id_praticien
    db.close()
...
def test_read_cheques_filter_patient_ilike(sec_headers, praticien_id):
    ...
    r = client.get(
        "/api/v1/cheques/", params={"id_patient": "p_test"}, headers=sec_headers
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
    assert r.json()[0]["id_patient"] == "P_TEST"
...
def test_read_cheques_filter_date_range(sec_headers, praticien_id):
    ...
    r = client.get(
        "/api/v1/cheques/", params={"date_to": "2023-03-01"}, headers=sec_headers
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_read_cheques_filter_montant(sec_headers, praticien_id):
    ...
    r = client.get(
        "/api/v1/cheques/", params={"montant_min": "100"}, headers=sec_headers
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_read_cheques_praticien_scope(prat_headers, sec_headers, praticien_id):
    ...
    db = TestingSessionLocal()
    autre = (
        db.query(models.Praticien).filter(models.Praticien.nom == "Dr. ScopeC").first()
    )
    assert autre is not None
    autre_id = autre.id_praticien
    db.close()
...
def test_read_journees_filter_date_range(sec_headers, praticien_id):
    ...
    r = client.get(
        "/api/v1/journees/", params={"date_from": "2023-06-01"}, headers=sec_headers
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_read_journees_filter_praticien(sec_headers, praticien_id):
    ...
    db = TestingSessionLocal()
    autre = (
        db.query(models.Praticien).filter(models.Praticien.nom == "Dr. ScopeJ").first()
    )
    ...
    r = client.get(
        "/api/v1/journees/", params={"id_praticien": praticien_id}, headers=sec_headers
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
    assert r.json()[0]["id_praticien"] == praticien_id
...
def test_read_journees_praticien_scope(praticien_id, sec_headers):
    ...
    db = TestingSessionLocal()
    autre = (
        db.query(models.Praticien).filter(models.Praticien.nom == "Dr. ScopeJ2").first()
    )
    assert autre is not None
    autre_id = autre.id_praticien
    db.close()
...
def test_x_total_count_reflects_filters(sec_headers, praticien_id):
    ...
    client.post("/api/v1/devis/", json=payload_acc, headers=sec_headers)
    r = client.get(
        "/api/v1/devis/", params={"statut": "EN_ATTENTE"}, headers=sec_headers
    )
    assert r.status_code == 200
    assert r.headers["x-total-count"] == "1"
    assert len(r.json()) == 1
...
def test_skip_limit_pagination(sec_headers, praticien_id):
    ...
    r = client.get(
        "/api/v1/devis/", params={"skip": 1, "limit": 1}, headers=sec_headers
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_read_charges_filter_designation(prat_headers, praticien_id):
    ...
    r = client.get(
        "/api/v1/charges/", params={"designation": "loyer"}, headers=prat_headers
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_read_charges_filter_periodicite(prat_headers, praticien_id):
    ...
    payload_ann = {
        **_charge_payload(praticien_id),
        "periodicite": "ANNUEL",
        "designation": "Taxe foncière",
    }
    client.post("/api/v1/charges/", json=payload_ann, headers=prat_headers)
    r = client.get(
        "/api/v1/charges/", params={"periodicite": "ANNUEL"}, headers=prat_headers
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_read_charges_filter_date_range(prat_headers, praticien_id):
    ...
    payload_late = {
        **_charge_payload(praticien_id),
        ...
        "designation": "Nouveau bail",
    }
    client.post("/api/v1/charges/", json=payload_late, headers=prat_headers)
    r = client.get(
        "/api/v1/charges/", params={"date_from": "2023-06-01"}, headers=prat_headers
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_read_charges_filter_montant(prat_headers, praticien_id):
    ...
    payload_cheap = {
        **_charge_payload(praticien_id),
        "montant": 50.0,
        "designation": "Petit abonnement",
    }
    client.post("/api/v1/charges/", json=payload_cheap, headers=prat_headers)
    r = client.get(
        "/api/v1/charges/", params={"montant_max": "500"}, headers=prat_headers
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
